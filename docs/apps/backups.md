...
AnaxExp provides backup mirroring to different storages. You can either use your own storage (currently only AWS S3 supported) or AnaxExp Storage.

### AnaxExp Storage

The simplest way to setup backup mirroring is to use simply select AnaxExp Storage as a provider. The default location is US east region if not specified differently. Backups older than a year (365 days) since the moment of upload will be deleted automatically.
