...
MariaDB can be configured with the following [environment variables](https://github.com/anaxexp/mariadb#environment-variables)
...
If you want to access the database outside of the AnaxExp infrastructure you will have to use SSH tunnel via the main container:
...
1. Set up SSH tunnel on port `53306` (you can change it). You can find `[SSH Port]` on `Instance > Stack > SSH` page. For MariaDB (port `3306` by default) use the following command:    
    ```shell
    $ ssh -L 53306:mariadb:3306 -p [SSH Port] anaxexp@[Server IP] -N
    ```
